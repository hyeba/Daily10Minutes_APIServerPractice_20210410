...
//    BaseActivity의 멤버 변수로 만들어두어야 각각의 화면에서 물려받아서 사용 가능
...
//        BaseActivity를 상속받는 클래스들은 => 화면이 onCreat 될 때 => 액션바 세팅도 되도록
//        무조건 세팅하는게 아니라, supportActionBar가 null 아닐 때(실체가 있을 때)만 실행 되도록

        supportActionBar?.let {

//            기본 액션바가 존재할 때만 let { } 내용이 실행 됨(코틀린 문법)
            setCustomActionBar()
        }
...
//        좌우 여백이 남아서 제거해야함
//        액션바를 잡고 있는(부모 역할) => Toolbar의 여백 제거(내부 공간을 0으로 설정)
